...
export default function Piechart({monthlyExpense}:ExpenseCharts){
    ...
    const total=monthlyExpense?.reduce((total,item)=>total + item.amount,0)||0
    ...
    return(
      ...
      <div className="bg-gray-800 border border-gray-700 p-6 rounded-3xl w-full lg:w-[450px] h-[350px] flex flex-col shadow-xl">
        ...
        <p className="text-red-400 font-semibold text-lg">Total: ₹{total.toFixed(2)}</p>
          ...
          <div className="flex-1 w-full h-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                {/* Pie registers list of colours and stores them in PieChart. Lagend Gets its data from that state */}
              </PieChart>
            </ResponsiveContainer>
          </div>
        )}
